...
The simplest way to start tracing is to call `init()`:
...
That's it. RDK automatically traces all LLM calls after `init()`.

### 3. Group calls with @observe (optional)

Use `@observe` to group multiple LLM calls into a single trace:
...
<Note>
Without `@observe`, each LLM call creates its own trace. With `@observe`, all calls within the function are grouped under a single trace.
</Note>
...
RDK automatically captures every LLM call (with or without `@observe`):

- **Token Usage** — Prompt, completion, and total tokens
- **Cost** — Automatic cost calculation in USD
- **Timing** — Start time, end time, and duration
- **Model Info** — Model name and provider
- **Cache Metrics** — Cache read/write tokens and savings
...
<Tip>
Cost tracking is automatic. See [Cost Tracking](/features/cost-tracking) for details on supported models and cost fields.
</Tip>
