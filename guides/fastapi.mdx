...
async def lifespan(app: FastAPI):
    ...
    print("RDK initialized")

    yield  # App runs here

    # Shutdown
    shutdown()
    print("RDK shutdown complete")
...
async def chat_with_request_id(
    ...
    # Request ID is now associated with this trace
    ...
```
...
async def chat_endpoint(request: ChatRequest) -> ChatResponse:
    try:
        ...
        # Span is marked as ERROR with this message
...
def disable_rdk():
    # Don't initialize RDK in tests
    os.environ["RDK_DISABLED"] = "true"
    yield
```
