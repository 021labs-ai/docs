...
RDK automatically instruments LangChain when initialized. Tracing works with or without `@observe`:
...
<Note>
Without `@observe`, each `invoke()` or `ainvoke()` call creates its own trace. With `@observe`, all calls within the function are grouped into a single trace.
</Note>
...
| `metadata.cost` | Cost breakdown (when using supported models) |

<Tip>
Cost tracking works automatically for LangChain calls using supported providers (Anthropic, OpenAI, Google). See [Cost Tracking](/features/cost-tracking) for details.
</Tip>
...
<Tip>
Use `@observe` at your top-level function to group all LangChain operations into a single trace. This makes it easier to analyze multi-step workflows.
</Tip>

1. **Group related calls** - Use `@observe` for multi-step pipelines; skip it for simple single calls
