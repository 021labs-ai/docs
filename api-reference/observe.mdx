...
<ParamField path="tool_mocks" type="ToolMocks" default="None">
  Mapping of tool names to mock outputs or callables. Active only within this function call.

  See [mock_tools()](/api-reference/mock-tools) for details.
</ParamField>

<ParamField path="tool_mock_default" type="Any" default="USE_GLOBAL_DEFAULT">
  Default mock response for any tool not in `tool_mocks`. Falls back to the process-wide default set by `mode="test"`.
</ParamField>
